...
class Criteria
{
     ...
     * @param $value    = valor
     */
    public function setProperty($property, $value,$sufix= null,$sufix2 = null)
    {
        
        if (isset($value))
        {   
            //Esquema de MAX , SUM em consulta sql
            $arr = ['max','sum','count','day','month','year'];
            ...
            if (in_array($property, $arr)) { 
                ...
                static $x = 0;
                if ($sufix != null){ 
                    $this->properties['crit'][$x]['property'] = $property;
                    $this->properties['crit'][$x]['sufix'] = $sufix;
                    $this->properties['crit'][$x]['valor'] = $value;
                }
                $x++;
            }elseif (in_array($property, $join)) {
                static $y = 0;
                //INNER JOIN
                if ($sufix != null){ 
                    $this->properties['join'][$y]['property'] = $property;
                    $this->properties['join'][$y]['banco'] = $value;
                    $this->properties['join'][$y]['criterio1'] = $sufix;
                    $this->properties['join'][$y]['criterio2'] = $sufix2;
                }
                $y++;
            }elseif($property == 'valor'){
                static $z = 0;
                    $this->properties['valor'][$z] = $value;
                $z++;
            }else{
                //ORDER BY, GROUP BY ..
